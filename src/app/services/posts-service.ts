...
import { CategoryColor, CategoryEnum } from './../models/category.enum';
...
export class PostsService {
  ...
  getRecentPosts(allPosts: Post[] = posts, limit: number = 3): Post[] {
    return [...allPosts]
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, limit);
  }

  getSuggestedPosts(category: CategoryEnum, currentPostId?: number): Post[] {
    let limit: number = 3;
    ...
    if (filteredPosts.length < limit) {
      const otherCategoryPosts = posts.filter((post) => {
        const isDifferentCategory = post.category !== category;
        const isNotCurrent = currentPostId ? post.id !== currentPostId : true;
        return isDifferentCategory && isNotCurrent;
      });

      // then we sort other posts by date
      const recentOtherPosts = [...otherCategoryPosts]
        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
        .slice(0, limit - filteredPosts.length);

      return [...filteredPosts, ...recentOtherPosts].slice(0, limit);
    }
    return filteredPosts.slice(0, limit);
  }
}
export const categories: Category[] = [
  { name: CategoryEnum.LIGHTING, count: 3, color: CategoryColor.EMERALD },
  { name: CategoryEnum.PORTRAIT, count: 3, color: CategoryColor.PURPLE },
  { name: CategoryEnum.LANDSCAPE, count: 2, color: CategoryColor.BLUE },
  { name: CategoryEnum.TECHNIQUES, count: 5, color: CategoryColor.ORANGE },
  { name: CategoryEnum.EQUIPMENT, count: 3, color: CategoryColor.EMERALD },
];
export const posts: Post[] = [
  {
    ...
    category: CategoryEnum.LIGHTING,
    author: {
      name: 'سالم أحمد',
      image:
  ...
  },
  {
    ...
    category: CategoryEnum.PORTRAIT,
    author: {
      name: 'محمد علي',
      image:
  ...
  },
  {
    ...
    category: CategoryEnum.LANDSCAPE,
    author: {
      name: 'إبراهيم حسن',
      image:
  ...
  },
  {
    ...
    category: CategoryEnum.TECHNIQUES,
    author: {
      name: 'داود خالد',
      image:
